...
- To run a script against synthetic data without the downloads, monkeypatch `cell_mnn.data.data_loading.get_data` to return `{"X_train": [array_per_timepoint...], "t_train": [...]}`. That covers every code path except preprocessing.
...
- **`--method ot-cfm` is broken**: `OTFlowMatchingDataset.__iter__` uses `self.rng`, which is never assigned (`src/cell_mnn/data/data_loading.py:222`). `i-cfm` and `batch-ot-cfm` work.
...
- `src/cell_mnn/__init__.py` re-exports `FlowMatchingDataset` and `MnnDataset` but not `get_datasets`, `TimeFilteredDataset`, or `SkipMarginalEvalDataset` — the useful entry points are import-by-path only.
- The `skip_idx` asserts are `1 <= skip_idx <= n_times - 1` in both `TimeFilteredDataset` and `SkipMarginalEvalDataset`, so the **final** timepoint *can* be held out. The assertion message in `SkipMarginalEvalDataset` (`data_loading.py:252`) has `t_skip` and `t_prev` swapped.
